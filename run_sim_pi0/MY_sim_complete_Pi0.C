...
#include <sys/stat.h>
int MY_sim_complete_Pi0(Int_t NO_Angle, Double_t Energy, string str_hz)
{
ostringstream out1,out2,out3;
out2 << NO_Angle;
out3 << fixed << setprecision(1) << Energy;
string str_NO_Angle = out2.str(), str_Energy = out3.str();
string path = "/home/jokey/Panda/data/WorkData_Pi0_A" + str_NO_Angle + "_E" +str_Energy  +"_" + str_hz;
int isCreate = ::mkdir(path.c_str(),S_IRUSR | S_IWUSR | S_IXUSR | S_IRWXG | S_IRWXO);
TString prefix = "/home/jokey/Panda/data/WorkData_Pi0_A" + str_NO_Angle + "_E" +str_Energy  +"_" + str_hz +"/evtcomplete";
Int_t nEvents = 10000;
...
string theta_min = os1.str(), theta_max = os2.str();
  ...
  TString inputGenerator = "box:type(111,1):p(" + str_Energy +"," + str_Energy  +"):tht("+ theta_min +","+ theta_min +"):phi(0,360)";
